    ...
    do <- function(f_idx){
        ...
        results <- mda.cache_load_or_run_save(D, f_idx, "zicoseq", {
            ZicoSeq.obj <- ZicoSeq(meta.dat = fdata$data, feature.dat = t(D$count_data), 
                    ...
                    max.abund.filter = 0.000, min.prop = 0, 
        ...
        res.full <- melt(as.data.frame(t(results$coef.list[[1]])) %>% rownames_to_column('taxa'), variable.name="variable.mda", value.name="effectsize", id.vars="taxa")
